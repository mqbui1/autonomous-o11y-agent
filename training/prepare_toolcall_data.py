...
def convert_capture(rec: dict, label_index: dict, toolspec_registry: dict) -> list[dict]:
    ...
    # Captures generated by a real Bedrock teacher run (agent_loop.py tags every
    # capture with the LLMProvider class name that produced it) are trusted as
    # gold distillation targets -- they never went through the auto_labeler's
    # rule-based approve/reject pass (that pipeline only scores the local
    # fine-tuned model's own generations), so an "unlabeled" join here means
    # "no human/rule label exists yet", not "quality unknown". Oversample them
    # the same 2x as an "approved" local capture, both on the final text turn
    # AND on intermediate tool-call turns (Bedrock's tool selection is itself
    # part of what we want to distill, not just its prose).
    is_teacher = rec.get("provider") == "BedrockProvider"
    ...
    for i, msg in enumerate(full):
        ...
        if is_final:
            ...
            if is_teacher and label == "unlabeled":
                row["_weight"] = 2
            else:
                row["_weight"] = _weight_count(label, scores)
        else:
            row["label"] = "unlabeled"
            row["_weight"] = 2 if is_teacher else 1  # tool-call turns always kept, regardless of final label
        rows.append(row)
    return rows
