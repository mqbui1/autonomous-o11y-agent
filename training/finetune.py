...
def run_mlx(args):
    ...
    print(f"  # IMPORTANT: do NOT overwrite {output_dir}/Modelfile with a bare 'FROM ...' line --")
    print("  # it must keep its TEMPLATE block (Qwen2.5 tool-calling chat template) or Ollama")
    print("  # silently falls back to a no-op '{{ .Prompt }}' template with no tool-call grammar,")
    print("  # which intermittently leaks raw <tool_call>...</tool_call> text as plain assistant")
    print("  # output instead of parsed tool calls (confirmed root cause, 2026-07-22 round 6).")
    print("  # Only update the FROM line in place:")
    print(f"  sed -i '' '1s|.*|FROM {output_dir}/o11y-agent-3b.gguf|' {output_dir}/Modelfile")
